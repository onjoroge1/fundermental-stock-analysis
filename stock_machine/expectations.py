...
def consensus_revision(rows: list[dict], as_of: str, window_days: int = 30) -> dict:
    cutoff_time = utc_timestamp(as_of)
    def available(row):
        if row.get("available_at"):
            return utc_timestamp(row["available_at"])
        # A legacy day-only observation is safe only after that day ends.
        return utc_timestamp(str(row["snapshot_date"])[:10]) + timedelta(days=1)
    eligible = [r for r in rows if r.get("snapshot_date")
                and available(r) <= cutoff_time
    ...
    if not eligible:
        return empty
    newest_day = max(str(r["snapshot_date"])[:10] for r in eligible)
    current = [r for r in eligible if str(r["snapshot_date"])[:10] == newest_day]
    # Prefer fiscal quarters, then the nearest period, then its latest known
    # observation. Missing precision in the legacy cache must not beat a
    # precise observation from the same day.
    current.sort(key=lambda r: (r["period_type"] != "quarter", str(r["forecast_period_end"]),
                               not bool(r.get("available_at")), -available(r).timestamp(),
                               str(r.get("source") or "")))
    current = current[0]
    prior_cutoff = cutoff_time - timedelta(days=window_days)
    previous = [r for r in eligible
                ...
                and available(r) <= prior_cutoff
                and available(r) < available(current)]
    prior = max(previous, key=available) if previous else {}
    result = {**empty, "forecast_period_end": current["forecast_period_end"],
              ...
              "snapshot_date": current["snapshot_date"], "prior_snapshot_date": prior.get("snapshot_date"),
              "source": current.get("source"), "available_at": available(current).isoformat(),
              "prior_available_at": available(prior).isoformat() if prior else None}
